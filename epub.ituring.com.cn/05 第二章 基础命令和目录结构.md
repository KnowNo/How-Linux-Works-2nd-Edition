...
<center><i>表2-1. 特殊字符</i></center>

|Character 字符|Name(s) 名称|Uses 用途|
|-|-|-|
|*|asterisk, star 星号|Regular expression 正则表达式, glob character 通用字符|
|￼.|dot 句点|Current directory 当前目录, file/hostname delimiter 文件/主机名的分隔符|
|!|bang 感叹号|Negation 逻辑非运算符, command history 命令历史|
|\||pipe 管道|Command pipes 命令管道|
|/|(forward) slash 斜线|Directory delimiter 目录分隔符, search command 搜索命令|
| \\ |backslash 反斜线|Literals 常量, macros (never directories) 宏（非目录）|
|$|dollar 美元符号|Variable denotation 变量符号, end of line 行尾|
| ' |tick, (single) quote 单引号|Literal strings 字符串常量|
|\`|backtick, backquote 反引号|Command substitution 命令替换|
|"|double quote 双引号|Semi-literal strings 半字符串常量|
|^|caret 脱字符|Negation 逻辑非运算符, beginning of line 行头|
|~|tilde, squiggle 波浪字符|Negation 逻辑非运算符, directory shortcut 目录快捷方式|
|#|hash, sharp, pound 井号|Comments 注释, preprocessor 预处理, substitutions 替换|
|[ ]|(square) brackets 方括号|Ranges 范围|
|{ }|￼braces, (curly) brackets 括号|Statement blocks 声明块, ranges 范围|
|_|underscore, under 下划线|Cheap substitute for a space 空格的简易替代|
...
<center><i>表2-2. 命令行键</i></center>

|按键|操作|
|-|-|
|￼CTRL-B|Move the cursor left 左移光标|
|￼CTRL-F|Move the cursor right 右移光标|
|￼CTRL-P|View the previous command (or move the cursor up) 查看上一条命令|
|￼￼CTRL-N|View the next command (or move the cursor down) 查看下一条命令|
|￼CTRL-A|Move the cursor to the beginning of the line 移动光标至行头|
|￼￼CTRL-E|Move the cursor to the end of the line 移动光标至行尾|
|￼￼CTRL-W|Erase the preceding word 删除前一个词|
|￼￼CTRL-U|Erase from cursor to beginning of line 删除从光标至行头的内容|
|￼￼CTRL-K|Erase from cursor to end of line 删除从光标至行尾的内容|
|￼￼CTRL-Y|Paste erased text (for example, from CTRL-U) 粘贴已删除的文本|
￼
##2.12 文本编辑器
...
帮助手册按照命令类型被组织为很多个章节，章节编号在章节名后面的括号中，例如：ping(8)。`表2-3`中列出了各章节和它们的编号。

<center><i>表2-3. 在线帮助手册章节列表</i></center>

| 章节 | 简介 |
|-|-|
|1| User commands 用户命令|
|2|System calls 系统调用|
|3|Higher-level Unix programming library documentation Unix高级编程库文档|
|4|Device interface and driver information 设备接口和设备驱动程序信息|
|5|File descriptions (system configuration files) 文件描述符（系统配置文件）|
|6|Games 游戏|
|7|File formats, conventions, and encodings (ASCII, suffixes, and so on) 文件格式，规范，和编码（ASCII编码和文件前缀等等）|
|8|System commands and servers 系统命令和服务|
...
<table>
  <tbody>
<tr> <td> ps x </td> <td> Show all of your running processes. 显示当前用户运行的所有进程 </td> </tr>
<tr> <td> ps ax </td> <td> Show all processes on the system, not just the ones you own. 显示系统当前运行的所有进程，包括其他用户的进程</td> </tr>
<tr> <td> ps u </td> <td> Include more detailed information on processes. 显示更详细的进程信息</td> </tr>
<tr> <td> ps w </td> <td> Show full command names, not just what fits on one line. 显示命令全名，而非仅显示一行以内的内容</td> </tr>
  </tbody>
</table>
...
<table>
<tbody>
￼<tr> <td> r </td> <td> Means that the file is readable. 文件可读</td> </tr>
￼<tr> <td> w </td> <td> Means that the file is writable. 文件可写</td> </tr>
￼<tr> <td> x </td> <td> Means that the file is executable (you can run it as a program). 文件可执行</td> </tr>
￼<tr> <td> - </td> <td> Means nothing. 无</td> </tr>
</tbody>
</table>
...
*创建符号链接的时候，请注意不要忘记-s选项。没有此选项的话，ln命令会创建一个硬链接（hard link），为文件创建一个新的名字。新文件拥有老文件的所有状态信息，和符号链接一样，打开这个新文件会直接打开文件内容。除非你掌握了`4.5 深入传统文件系统`一节的内容，否则不要使用符号链接。*
